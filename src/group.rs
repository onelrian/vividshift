...
pub fn distribute_work(
    ...
    for &(task, num_required) in &work_definitions {
        // Step 1: Find all eligible candidates from the unassigned pool for this specific task.
        ...
        // Step 4: Update the state for the next iteration.
    ...
    if !unassigned_people.is_empty() {
        ...
        people_to_place.shuffle(&mut thread_rng()); // Shuffle for random placement
